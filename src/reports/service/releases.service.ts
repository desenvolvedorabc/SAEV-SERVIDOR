...
export class ReleasesService {
  ...
  async handle(paginationParams: PaginationParams, user: User) {
    ...
    if (school) {
      queryAsses.andWhere("schools.ESC_ID = :school", { school });
    } else if (county) {
      queryAsses.andWhere("AVM_MUN.MUN_ID = :county", { county });
    }
    ...
    if (school) {
      queryReports.andWhere("TUR_ESC.ESC_ID = :school", { school });
    } else if (county) {
      queryReports.andWhere("ESC_MUN.MUN_ID = :county", { county });
    ...
    const totalByFilter = {
      total_schools: county && !school ? data.meta.totalItems : 0,
      total_mun: edition && !county ? data.meta.totalItems : 0,
      total_schoolClasses:
        (school && !schoolClass && data.meta.totalItems) || (schoolClass && 1),
    };
