...
export class PerformanceLevelService {
  ...
  async handle(paginationParams: PaginationParams, user: User) {
    ...
    if (schoolClass) {
      ...
      if (school) {
        queryBuilder.andWhere("schools.ESC_ID = :school", { school });
      } else if (county) {
        queryBuilder.andWhere("AVM_MUN.MUN_ID = :county", { county });
      }
    ...
    const queryReports = this.reportEditionRepository
      ...
      .leftJoinAndSelect("reportsSubjects.test", "testSubject")
      .innerJoinAndSelect("reports_descriptors.descriptor", "descriptor")
      .leftJoinAndSelect("reports_descriptors.test", "test")
    ...
    if (school) {
      queryReports
        ...
        .andWhere("TUR_ESC.ESC_ID = :school", { school });
    } else if (county) {
      queryReports
        .leftJoinAndSelect("report.school", "school")
        .leftJoin("school.ESC_MUN", "ESC_MUN")
        .andWhere("ESC_MUN.MUN_ID = :county", { county });
    ...
    if (reports_editions?.length) {
      formattedSubjects = subjects.map((subject) => {
        ...
        const items = reports_editions.map((report_edition) => {
          ...
          const reportSubject = report_edition.reportsSubjects.find((subject) => subject.test.TES_ID === descriptors[0]?.idTest)
