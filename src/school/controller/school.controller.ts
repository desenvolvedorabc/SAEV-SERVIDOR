...
export class SchoolController {
  ...
  async paginate(
    ...
    @Query()
    { page, limit, search, order, status, column, county, active }: PaginationParams,
  ): Promise<Pagination<School>> {
    limit = Number(limit) > 100 ? 100 : limit;
    return this.schoolService.paginate(
      {
        page: +page,
        limit: +limit,
        route: " ",
  ...
  getSchoolsReport(
    ...
    return this.schoolService.getSchoolsReport(paginationParams);
  }
  ...
  @ApiBearerAuth()
  @Get("/county/:id")
  findByCounty(@CurrentUserId() userId: string, @Param() {id}: IdQueryParamDto) {
    return this.schoolService.findByCounty(userId, String(id));
  }
