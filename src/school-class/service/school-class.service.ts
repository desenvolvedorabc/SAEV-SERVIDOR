...
export class SchoolClassService {
   ...
   */
  findBySchoolAndSerie(ESC_ID: string, series: string, active?: "0" | "1") {
    if (!!series) {
      if (active) {
        return this.schollClassRepository.find({
          where: {
            TUR_ESC: { ESC_ID: ESC_ID },
            TUR_SER: { SER_ID: In(series?.split(",")) },
            TUR_ATIVO: active,
          },
          relations: ["TUR_ESC", "TUR_SER", "TUR_MUN"],
          order: { TUR_NOME: "ASC" },
        });
      }
    ...
    if (active) {
      return this.schollClassRepository.find({
        where: { TUR_ESC: { ESC_ID: ESC_ID }, TUR_ATIVO: active },
        relations: ["TUR_ESC", "TUR_SER", "TUR_MUN"],
        order: { TUR_NOME: "ASC" },
      });
    }
  ...
  async add(createSchoolClassDto: CreateSchoolClassDto, user: User) {
   const {schoolClass} = await this.verifyExistsSchoolClass(createSchoolClassDto);
  ...
  async update(
    ...
    const { schoolClass } = await this.verifyExistsSchoolClass(updateSchoolClassDto);
