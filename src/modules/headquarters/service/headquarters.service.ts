...
import { ISubject } from 'src/modules/subject/model/interface/subject.interface'
...
export class HeadquartersService {
  ...
  async findOne(MAR_ID: number) {
    const main = await this.headquarterRepository.findOne(
      { MAR_ID },
      { relations: ['MAR_DIS', 'MAR_SER'], order: { MAR_SER: 'ASC' } },
    )
    const getTopics = await this.findTopics(main.MAR_ID)
    const topics = await Promise.all(
      getTopics.map(async (topic) => {
        const item = await this.findTopicsItems(topic.MTO_ID)
        topic.MTO_MTI = item
        return topic
      }),
    )
  ...
  async toggleActiveDescriptor(id: number): Promise<{ active: boolean }> {
    ...
    if (!active && !!descriptor?.testsTemplate?.length) {
      throw new ForbiddenException(
        'Você não pode inativar um descritor que está em uma prova.',
      )
    }
