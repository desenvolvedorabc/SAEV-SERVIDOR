...
export class AssessmentOnlineService {
  ...
  async create(createAssessmentOnlineDto: CreateAssessmentOnlineDto) {
    ...
    const assessmentOnline = this.assessmentOnline.create({
      test,
    })
    ...
    try {
      ...
      for (const page of pages) {
        ...
        for (const question of page.questions) {
          ...
          await queryRunner.manager.save(AssessmentOnlineQuestion, saveQuestion)
  ...
  async getStudentsForAssessmentOnline(
    ...
    const students = await Promise.all(
      data.items.map(async (student) => {
        const isAvailable = await this.verifyStudentExistsInTest(
          student.ALU_ID,
          test.TES_ID,
        )
  ...
  async verifyStudentExistsInTest(studentId: number, testId: number) {
