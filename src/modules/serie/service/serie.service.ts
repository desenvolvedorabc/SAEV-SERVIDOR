...
import { EvolutionaryLineService } from 'src/modules/reports/service/evolutionary-line.service'
...
export class SerieService {
  ...
  async paginate(
    ...
    const queryBuilder = this.seriesRepository.createQueryBuilder('SERIE')

    if (active !== undefined && active !== null) {
      const activeFilter = +active
      queryBuilder.where('SERIE.SER_ATIVO = :activeFilter', { activeFilter })
    }

    if (search?.trim()) {
      queryBuilder.andWhere('SERIE.SER_NOME LIKE :q', { q: `%${search}%` })
    }
